...
![入力フィールド内の null 数式](~@img/formula-docs/null-formula.png)
...
![入力フィールド内の clear 数式](~@img/formula-docs/clear-formula.png)
...
このフィールドに対して、宛先アプリに何も渡しません。フィールドに既存の値がある場合、そのままにしておきます。
...
![入力フィールド内の skip 数式](~@img/formula-docs/skip-formula.png)
_skip 数式の使用例_
...
文字列を URL デコードします。この数式では、URL デコードに `CGI.unescape` を使用します。
...
JSON Web Token（JWT）を指定されたアルゴリズム（RS256、RS384、RS512、HS256、HS384、HS512、ES256、ES384、またはES512）を使用してデコードします。
...
以下のアルゴリズムのいずれかを使用して、Jason Web Token (JWT) を作成します - RS256、RS384、RS512、HS256、HS384、HS512、ES256、ES384、または ES512。次の例では、ヘッダーに `kid` のような他の名前付きパラメータを追加します。
...
この式では、Workato のルックアップテーブルからキーを使用して値を検索することができます。大文字と小文字、データ型に注意してください。

ルックアップ式でデータピルを使用する場合、データが適切な形式に変換されることをお勧めします。たとえば、整数型のデータピルは、整数と文字列の両方を含む列と比較する場合、`.to_s` 式で文字列に変換する必要があります。

### 例
たとえば、次の名前が `Department Codes` で ID が 6 のルックアップテーブルを使用します。
...
| `lookup('Department Lookup table', 'Department': 'Marketing')['Department Code']`    | nil <br>大文字と小文字が区別されるため、列 "Department Code" が見つかりません|
| `lookup('6', 'Department code': 'ACC')['Department']`                         | "会計"  <br>注意: ルックアップテーブル ID を引用符 `""` で囲むことを忘れないでください。 |

::: tip ルックアップ ID の使用
ルックアップテーブル名とルックアップテーブル ID は互換性があります。ルックアップテーブル ID は URL から取得できます。
